...
class Neuralnet:
    ...
    def feedforward(self, inputs):
        ...
        for layer in range(self.layers - 1):
            ...
            self.inputs[layer + 1] = np.dot(self.weights[layer].T,
                                            self.outputs[layer]) + self.biases[layer]
    ...
    def backpropagate(self, targets):
        ...
        for layer in reversed(range(self.layers - 1)):
            ...
            self.errors[layer] = gradient * \
                np.dot(self.weights[layer], self.errors[layer + 1])
        # Adjust weight matrices and bias vectors
        for layer in range(self.layers - 1):
            self.weights[layer] -= self.rate * \
                np.outer(self.outputs[layer], self.errors[layer + 1])
...
for digit in range(10):
    with open('digits/' + str(digit), 'r') as digitfile:
        digits.append(np.fromfile(
            digitfile, dtype=np.uint8).reshape(1000, 28, 28))
